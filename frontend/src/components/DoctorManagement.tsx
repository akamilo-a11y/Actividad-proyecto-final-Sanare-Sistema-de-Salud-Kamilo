...
import { Plus, Edit, Trash2, X, UserPlus, Mail, Phone, Building, Stethoscope, RefreshCw } from 'lucide-react';
...
export default function DoctorManagement() {
  ...
  const handleGenerateSlots = async (id: string) => {
    try {
      setGenerating(id);
      setError('');
      const result = await doctorService.generateSlots(id);
      const doctor = await doctorService.getById(id);
      setDoctors((prev) =>
        prev.map((d) => (d.id === id ? { ...d, availableSlots: doctor.availableSlots } : d)),
      );
      alert(`Se generaron ${result.generated} horarios disponibles`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al generar horarios');
    } finally {
      setGenerating(null);
    }
  };
    ...
    <div className="space-y-6">
            ...
            {doctors.map((doctor) => (
              <tr key={doctor.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button
                    onClick={() => handleGenerateSlots(doctor.id)}
                    disabled={generating === doctor.id}
                    className="text-emerald-600 hover:text-emerald-900 mr-4"
                    title="Generar horarios disponibles"
                  >
      ...
      <div className="lg:hidden space-y-4">
        {doctors.length === 0 ? (
          ...
          doctors.map((doctor) => (
            <div key={doctor.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              ...
              <div className="flex gap-2 pt-3 border-t border-gray-100">
                <button
                  onClick={() => handleGenerateSlots(doctor.id)}
                  disabled={generating === doctor.id}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors"
                >
                  <RefreshCw className={`w-4 h-4 ${generating === doctor.id ? 'animate-spin' : ''}`} />
